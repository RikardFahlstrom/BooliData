...
def main():
    output = create_api_request('API_CALLERID', 'API_KEY', '386690,870600')
    df = create_df(output)
    html = create_html(df)
    send_email('rikard.fahlstrom@gmail.com', 'rikard.email.notification@gmail.com', 'EMAIL_PW', html)
...
def create_api_request(api_callerid, api_key, area):
    ...
    areaId = area
    offset = '0'
    limit = str(15)  # Maximum number of response objects. Limit = 500
